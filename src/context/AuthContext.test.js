      ...
      const LoginTrigger = () => {
        ...
        return (
          <>
            <span data-testid="user-status">{user ? "logged-in" : "logged-out"}</span>
            <span data-testid="token-val">{token || "no-token"}</span>
            <button data-testid="login-btn" onClick={() => login("bob@example.com", "password123")}>
              Login
            </button>
          </>
        );
      ...
      const LoginTrigger = () => {
        ...
        return (
          <div>
            ...
            <button data-testid="login-btn" onClick={() => login("bad@example.com", "wrong")}>
              Login
            </button>
          </div>
        );
  ...
  describe("isAuthenticated", () => {
    it("returns true when user is authenticated with cookie-managed token", async () => {
      ...
      render(
        <AuthProvider>
          <AuthValueProbe
            compute={({ isAuthenticated }) => isAuthenticated()}
            onResult={onResult}
          />
        </AuthProvider>
      );
